---
title: "ProjecTME Complete Vignettes"
...
opts_chunk$set(echo = F,cache = F,prompt = F,tidy = 'styler',comment = NA,dpi = 300,
...
## **1.Single-cell gene signature scoring with UCell (UCell signature enrichment analysis)**
...
```{r,eval=FALSE}
library(GEOquery)
if(!file.exists('./data/GSE115978.rds.gz')){
  geo_acc <- 'GSE115978'
  ...
  exp.mat <- readr::read_rds('./data/GSE115978.rds.gz')
}
```
...
knitr::include_graphics("./plots/ucell.plot.1.png")
```
...
Then, we can apply our signature set, or any other new signature to the pre-calculated ranks.
The calculations will be considerably faster.
```{r,eval=FALSE}
set.seed(123)
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.2.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.3.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.4.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.5.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.7.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.8.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.9.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.10.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.11.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.12.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.13.png")
```
...
b <- FeaturePlot(pbmc.Tcell,reduction = 'wnn.umap',features = c('Tcell_CD8_UCell','Tcell_CD8_Seurat1'),
                 ncol = 2,order = T)
cowplot::plot_grid(plotlist = list(a,b),nrow = 1,rel_widths = c(1,2))
```

```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.14.png")
```
...
b <- FeaturePlot(pbmc.Tcell.CD8,reduction = 'wnn.umap',features = c('Tcell_CD8_UCell','Tcell_CD8_Seurat1'),
                 ncol = 2,order = T)
cowplot::plot_grid(plotlist = list(a,b),nrow = 1,rel_widths = c(1,2))
```

```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.15.png")
```

## **3.Using UCell and Seurat to identify different T cell subtypes/states in human tumors (Evaluating human TIL subtype signatures using UCell)**

Use UCell gene signature scoring to identify and filter tumor-infiltrating T cell subtypes.

### **Query single-cell data**

Use data from Yost et al. Nat Med 2019 Pre-compiled R object.
```{r,eval=FALSE}
inputFile <- './data/Yost.pretreatment.all.rds'
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.16.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.17.png")
```
...
```{r,eval=FALSE}
signaturesHumanCellTypes <- readRDS('./data/signaturesHumanCellTypes.rds')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.18.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.19.png")
```

We can appreciate that some clusters have a clearly high mean distribtion of T cell signature scores (T.cell_UCell score >0.2). We can use this value as a rudimentary threshold to split T cell from non-T cell clusters.

### **Identify T cells based on signatures and subset them**

Identify T cell clusters by UCell score, select as Tcell clusters only those with median UScore > 0.2.
```{r,eval=FALSE}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.20.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.21.png")
```
...
```{r,eval=FALSE}
signatureHumanTILs <- readRDS('./data/signaturesHumanTILs.rds')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.22.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.23.png")
```
...
heatmap(t(TILsigScores_vs_OriginalCluster.m),cexCol = 1.2, cexRow = 1.2, scale = 'none')
```

```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ucell.plot.24.png")
```
...
DimPlot(seu.obj, group.by = 'is.pure')
```

```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.1.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.2.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.3.png")
```

## **scGate as a multi-class classifier**

scGate can also be used as a cell type classifier, to annotate multiple cell types in a dataset. scGate can also be used as a cell type classifier, to annotate multiple cell types in a dataset.
```{r,eval=FALSE}
models.lits <- scGate_models_DB$human$generic[c('Bcell','MoMacDC','CD8T','CD4T','Erythrocyte')]
seu.obj <- scGate(seu.obj, model = models.lits)
```

## **1.scGate as a multi-class classifier**
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.4.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.5.png")
```
...
With this very simple model, >99% (Recal) are isolated with a >99% purity (Precision), and overall classification performance (MCC) > 99%. Another example: we can isolate plasmacytoid dendritic cells (pDCs), defined using the marker LILRA4 (e.g. https://pubmed.ncbi.nlm.nih.gov/30395816/).
```{r,eval=FALSE}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.6.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.7.png")
```
...
In the examples above, we have been using a single-cell dataset derived from blood (PBMC). When working with more complex tissues, such as tumors, we might need to apply multiple levels of gating to isolate the population of interest, scGate allows to purify by steps, using a hierarchical gating model- for example, first purifying immune cells, and among the immune cells the cell type of interest. Let's explore (a downsampled version of) the whole tumor dataset by [Jerby-Arnon et al., 2018](https://pubmed.ncbi.nlm.nih.gov/30388455/), with the cell type annotations provided by the authors.
```{r,eval=FALSE}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.8.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.9.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.10.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.11.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.12.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.15.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.16.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.17.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.18.png")
```
...
scGate can also be used as a cell type classifier, to annotate multiple cell types in a dataset. Simply provide a list of models (one for each cell type of interest) to scGate and these will be jointly evaluated (faster than computing them individually).
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.19.png")
```
...
By default, scGate performs dimensionality reduction (DM) at each hierarchical level of the gating strategy, for sensitive kNN smoothing. However, if a pre-calculated DM is available in your input object, this can be provided to scGate to avoid this step and speed-up computations. This feature can be particularly useful to gate integrated datasets (eg after suing Harmony, Seurat or STACAS).
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.20.png")
```

## **2.scGate to annotate integrated scRNA-seq datasets**
...
library(dplyr)
library(harmony)
pbmcsca <- readRDS('./data/pbmcsca.rds')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.21.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.22.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.23.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.24.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.25.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.26.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.27.png")
```

## **3.Isolating target cell types from scATAC-seq data using scGate**

### **Introduction**

This part illustrates the application of scGate to isolate different target cell population from scATAV-seq data.
...
```{r,eval=FALSE}
inputdata.10x <- Read10X_h5('data/pbmc_gra_atac/pbmc_granulocyte_sorted_10k_filtered_feature_bc_matrix.h5')
...
frag.file <- sprintf('data/pbmc_gra_atac/pbmc_granulocyte_sorted_10k_atac_fragments.tsv.gz')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.28.png")
```
...
The `GeneActivity()` function from Signac allows calculating per-gene counts from ATAC counts.
We will store this information in the `GeneActivity` assay.
```{r,eval=FALSE}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.29.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.30.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.31.png")
```
...
```{r,eval=FALSE}
# To verify presence of genetic signals, and test out signatures for target cell types, we can run UCell
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.32.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.33.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.34.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.35.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.36.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.37.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.38.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.39.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.40.png")
```

## **4.Isolating target cell types from CITE-seq data using scGate**
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.41.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.42.png")
```

The surface protein measurements for key B cell markers CD19 and CD20 (stored in the ADT assay) confirm that scGate could isolate pure B cells from transcriptomics data.
```{r,eval=FALSE}
VlnPlot(hao,features = c('CD20','CD19'),assay = 'ADT',cols = c(list('Impure'='gray','Pure'='green')),pt.size = 0)
```

```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.43.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.44.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.45.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.46.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.47.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.48.png")
```

The distribution in bimodal, but the default threshold `pos.thr = 0.2` is too strict.
```{r,eval=FALSE}
t.adt <- gating_model(name = 't.adt', signature = c('CD3-1','CD3-2','CD41-'))
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.49.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.50.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.51.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.52.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.53.png")
```

Finally, we try to detect Platelets using ADT counts. Since CD41 could be expressed in monocytes and dendritic cells, we need to add a negative signature. Positive and negative signatures could present different UCell distributions when dealing with ADT data; again we need to adjust the thresholds for positive and negative UCell scores (the distribution plots can help you set a threshold).
```{r,eval=FALSE}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/scgate.plot.54.png")
```

# **ProjecTME: A framework for reference-based single-cell RNA-seq data analysis**

ProjecTME is a computational method to project scRNA-seq data into reference single-cell atlaes, enabling their direct comparison in a stable, annotated system of coordinates.
...
## **DEMO: Projecting scRNA-seq data onto a reference map**
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.1.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.2.png")
```
...
querydata2 <- read.sc.query(filename = './data/GSE86028_TILs_sc_wt_mtko.tpm.log2.txt.gz',type = 'raw.log2')
```
...
# NB: by default, Run.ProjecTME() will pre-filter T cell using scGate. In case the input dataset is
# already pre-filtered, we can disable this step using Run.ProjecTME(querydata1,ref=ref,filter.cells=F).
# If we are using a custom reference map that is not composed of T cells, we may specify a different
# scGate filter using the scGate_model parameter.
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.3.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.4.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.5.png")
```
...
# We obtain per-cell scores using UCell, and then generate radar plots on average signature
# scores per subtype
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.6.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.7.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.8.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.9.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.10.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.11.png")
```
...
## **Building a custom reference atlas for ProjecTME**
...
```{r,eval=FALSE}
meta <- read.delim('data/Zilionis/GSE127465_mouse_cell_metadata_15939x12.tsv.gz',header = T)
meta$Barcode_library <- paste(meta$Barcode, meta$Library, sep = '_')
rownames(meta) <- meta$Barcode_library
exp.mat <- Matrix::readMM('data/Zilionis/GSE127465_mouse_counts_normalized_15939x28205.mtx.gz')
gene.list <- read.csv('data/Zilionis/GSE127465_gene_names_mouse_28205.tsv.gz',header = F)$V1
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.12.png")
```

### **Generate ProjecTME reference**

We are now ready to convert the dataset into a ProjecTME reference. The make.reference function will populate the required metadata fields and return an object that should be directly usable as a reference.
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.13.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.14.png")
```
...
query.projected <- Run.ProjecTME(query, ref = ref.momac, filter.cells = F, skip.normalize = T)
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.15.png")
```
...
## **1.MC38 TILs with clonotype analysis**
...
exp_mat <- Read10X('data/Xiong_TILs/')
...
for(i in 1:length(libIDtoSampleID_VDJ)){
  ...
  vdj.list[[i]] <- read.csv(paste0('data/Xiong_TILs/TCR/filtered_contig_annotations_',s,'.csv'),as.is = T)
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.17.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.18.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.19.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.20.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.21.png")
```
...
meta <- reshape2::melt(
  table(query.projected@meta.data[!is.na(query.projected$Frequency),c('functional.cluster','cloneType')]),
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.22.png")
```

This confirms the initial observation that Tex and Tpex present the largest clonal expansion.
We can highligh specific clonotypes on the reference atlas (here those with at least 40 cells).
select column/variable to use as clonotype ID, in this case CTaa, the paired TCR CDR3 aminoacid sequences.
```{r,eval=FALSE}
clone_call <- 'CTaa'
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.23.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.24.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.25.png")
```
...
The combination of ProjecTME and scRepertoire simplifies the joint analysis of single-cell expression data and clonotype analysis, in the context of an annotated reference atlas of TIL states.


## **2.Project human data on mouse TIL atlas**

**TIL contexture predicts checkpoint blockade response in melanoma patients**

In this case study, we will use ProjecTME to interpret human scRNA-seq T cell data in the context of a murine TIL atlas. We are going to use the single-cell dataset generated by Sade-Feldman GSE120575 to illustrate interspecies mapping of human cells on a stable murine atlas using gene orthologs.

### **Background**

It remains unclear why some patients respond to chekpoint blockade therapy while others do not. In the study by Sade-Fedman et al. (2018) Cell, the authors characterize transcriptional profiles of immune cells from melanoma patients before and after immune checkpoint blockade, with the goal to identify factors that associate with success or failure of checkpoint therapy. They reported that the balance between two CD8 T cell states found in tumor tissue (essentially, memory-like vs exhausted-like) is linked to tumor regression following checkpoint blockade. And in particular, that the frequency of TCF7+CD8+T cells in tumors predicts response and better survival.

The single-cell expression data GSE1250575 consists of CD45+ single cells from 48 tumor biopsies of melanoma patients before or after checkpoint inhibitors treatment, sequenced using the Smart-seq2 protocol. Meta-data on response to therapy (RespondeR vs Non-responder) is also available on the same GEO identifier.
```{r,eval=FALSE}
library(Seurat)
library(dplyr)
library(ggplot2)
...
```{r,eval=FALSE}
if(!file.exists('data/SadeFeldman/SadeFeldmen.seurat.rds')){
  exp.mat <- read.delim('data/SadeFeldman/GSE120575_Sade_Feldman_melanoma_single_cells_TPM_GEO.txt.gz',header=F,sep='\t')
  ...
  meta <- read.delim('data/SadeFeldman/GSE120575_patient_ID_single_cells.txt.gz',
  ...
  saveRDS(query.object, file = 'data/SadeFeldman/SadeFeldmen.seurat.rds')
}else{
  query.object <- readRDS('data/SadeFeldman/SadeFeldmen.seurat.rds')
}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.26.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.27.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.28.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.29.png")
```
...
## **3.T cell diversity across multiple tissues**
...
```{r,eval=FALSE}
data_dir <- 'data/Sandu/P14_tissues/'
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.30.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.31.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.32.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.33.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.34.png")
```

### **Discriminant gene analysis**

The ProjecTME analysis showed that different tissues are composed of different proportions of broad T cell states. However, we may also ask whether there is variability between cells within specific reference T cell states. For instance, do effector (SLEC) cells from blood differentially express genes compared to spleen SLEC cells? 
do terminally exhausted (Tex) cells from liver differ from Tex cells from spleen?
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.35.png")
```
...
## **4.Exhausted CD8 T cells are associated with ICB responsiveness in breast cancer**
...
cohort1 <- readRDS('data/Bassez_Tcell_data/1864-counts_tcell_cohort1.rds')
meta1 <- read.csv('data/Bassez_Tcell_data/1870-BIOKEY_metaData_tcells_cohort1_web.csv')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.36.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.37.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.38.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.39.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.40.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.41.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.42.png")
```
...
## **5.Memory phenotype of CD4+ T cells after acute infection**
...
### **scRNA-seq data prepration**
```{r,eval=FALSE}
exp.mat.1 <- read.table('data/Kunzli/GSE134157_UMImatrix_NICD_protector.tsv.gz')
exp.mat.2 <- read.table('data/Kunzli/GSE134157_UMImatrix_no_NICD_protector.tsv.gz')
# Genes in these matrices are given by Ensembl ID - we will convert them to gene names.
table <- read.csv('data/Kunzli/mart_export_Mmu_021220.txt.gz',sep = '\t')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.43.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.44.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.45.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.46.png")
```
...
## **6.Detecting novel/unrepresented cell states**
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.47.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.48.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.49.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.50.png")
```
...
### **Recalculate map with novel states**
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.51.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.52.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.53.png")
```

This constitutes an updated reference that you can use directly to project and interpret new data.


## **7.Effect of anti-PD-L1 therapy on CD4+T cells in chronic infection**
...
iso <- Read10X('data/Snell/CD4_ISO/')
pdl <- Read10X('data/Snell/CD4_PDL/')
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.54.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.55.png")
```
...
ProjecTME can be used in two models: 1) as a classifier, to annotate query cells in their original space; 2) as a method to embed the query dataset into the same space of the reference map. We will apply both below.

**ProjecTME classifier**
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.56.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.57.png")
```

**ProjecTME reference embedding**

The `Run.ProjecTME` function integrates the query with the reference, transfers cell type labels from reference to query, and embeds the cells from the query in the same low-dimensional embeddings (PCA and UMAP) of the reference. This allows comparing different conditions in the same system of coordinates.
```{r,eval=FALSE}
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.58.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.59.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.60.png")
```
...
```{r,echo=FALSE,out.width="100%"}
knitr::include_graphics("./plots/ProjecTME.plot.61.png")
```
